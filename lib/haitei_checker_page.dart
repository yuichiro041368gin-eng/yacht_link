...
class _YachtForm {
  String yachtClass = 'スナイプ';
  Sailor? skipper;
  List<Sailor?> crews = [null];
}

/// ドラッグ中の人員情報。
/// 移動元スロットへの書き戻し用セッターを持ち、空きスロットへのドロップは「移動」、
/// 埋まっているスロットへのドロップは「入れ替え」になる。
class _DragSailor {
  final Sailor sailor;
  final void Function(Sailor?) setSource;
  _DragSailor(this.sailor, this.setSource);
}
...
class _HaiteiCheckerPageState extends State<HaiteiCheckerPage> {
  bool _loading = true;
  String? _loadError; // 読み込み失敗時のメッセージ（再読み込みボタンを表示する）
  String? _teamId;
  String _teamName = '';
  ...
  Future<void> _loadMembers() async {
    ...
    if (user == null) {
      // ここでスピナーを止めないと読み込み中のまま固まる
      if (mounted) {
        setState(() {
          _loading = false;
          _loadError = 'ログイン情報を確認できませんでした';
        });
      }
      return;
    }
    try {
      ...
      final memberFuture = FirebaseFirestore.instance
          ...
          .get()
          .timeout(const Duration(seconds: 15));
      final boatsFuture = _loadRescueBoats(teamId);

      final snap = await memberFuture;
      final members =
          snap.docs.map((d) => Sailor.fromMap(d.id, d.data())).toList()
            ..sort((a, b) => a.name.compareTo(b.name));
      final boats = await boatsFuture;
      ...
      if (mounted) {
        setState(() {
          _loading = false;
          _loadError = 'データの読み込みに失敗しました。通信環境を確認して再読み込みしてください。';
        });
      }
    }
  }

  // 機材ページで登録されたレスキュー艇を読み込む（失敗しても空リストで続行）
  Future<List<_RescueBoatOption>> _loadRescueBoats(String teamId) async {
    try {
  ...
  Color _verdictColor(List<Finding> findings) {
    if (findings.any((f) => f.severity == Severity.stop)) {
  ...
  void _showResults(
    Conditions cond,
    List<YachtPlan> yachts,
    List<RescueBoatPlan> rescues,
    List<Sailor> shoreStaff,
                        ...
                        onPressed: () async {
                          ...
                          if (context.mounted) {
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(
                                content: Text(
                                  '配艇報告テキストをコピーしました（OB会への配艇送付などに使えます）',
                                ),
                              ),
                            );
                          }
  ...
  String _buildReportText(
    ...
    for (var i = 0; i < yachts.length; i++) {
      ...
      final skipper = y.skipper != null
          ...
          : '未定';
      final crews = y.crews.isEmpty
          ? '未定'
          : y.crews.map((c) => c.name).join('、');
    ...
    final violations = findings
        .where((f) => f.severity == Severity.violation)
        .length;
  ...
  Future<void> _savePlan(
    ...
    try {
      await FirebaseFirestore.instance.collection('haitei_plans').add({
        ...
        'stopCount': findings.where((f) => f.severity == Severity.stop).length,
                          ...
                          showDialog(
                            ...
                            builder: (context) => AlertDialog(
                              ...
                              content: SingleChildScrollView(
                                child: SelectableText(data['reportText'] ?? ''),
                              ),
                              actions: [
                                TextButton(
                                  onPressed: () async {
  ...
  /// スロットを長押しドラッグで移動・入れ替えできるようにするラッパー。
  /// [selected] が入っていれば長押しでドラッグを開始でき、他のスロットへ
  /// ドロップすると移動（相手が埋まっていれば入れ替え）になる。
  /// [onChanged] はドロップ時にこのスロットへ人員を書き込むためのセッター。
  Widget _dragDropSlot({
    required Sailor? selected,
    required void Function(Sailor?) onChanged,
    required Widget child,
  }) {
    Widget content = child;
    if (selected != null) {
      content = LongPressDraggable<_DragSailor>(
        data: _DragSailor(selected, onChanged),
    ...
    return DragTarget<_DragSailor>(
      // 自分自身へのドロップは何もしない
      onWillAcceptWithDetails: (details) =>
          details.data.sailor.id != selected?.id,
      onAcceptWithDetails: (details) {
        final prev = selected;
        setState(() {
          onChanged(details.data.sailor);
          details.data.setSource(prev); // 相手が空ならnullが入り「移動」になる
        });
      },
      builder: (context, candidates, rejected) {
        // ドラッグ中のホバー先を枠線で示す（foregroundDecorationでレイアウトを崩さない）
        return Container(
    ...
    return _card(
      ...
      children: [
        const Padding(
          ...
          child: Text(
            '💡 配置した名前を長押しするとドラッグで移動できます（レスキュー・陸上要員との間もOK。相手がいる場所へ落とすと入れ替え）',
        ...
        children: [
          ...
          for (var c = 0; c < y.crews.length; c++)
            _memberTile(
              ...
              onChanged: (s) => y.crews[c] = s,
            ),
          TextButton.icon(
            icon: const Icon(Icons.add, size: 16),
            label: const Text('クルーを追加（体験乗船など）', style: TextStyle(fontSize: 12)),
  ...
  Future<void> _pickRescueBoat(_RescueForm r) async {
    if (_boatOptions.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('機材ページの「レスキュー」カテゴリに艇を登録すると、ここから選択できます')),
      );
      return;
    }
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
              ...
              items: RescueRole.values
                  .map(
                    (role) => DropdownMenuItem(
                      value: role,
                      child: Text(rescueRoleLabels[role]!),
                    ),
                  )
                  .toList(),
          ...
          Expanded(
            child: _dragDropSlot(
              selected: slot.sailor,
              onChanged: (s) => slot.sailor = s,
              child: InkWell(
                onTap: () async {
                  final s = await _pickMember(
                    '${rescueRoleLabels[slot.role]}を選択',
                  );
